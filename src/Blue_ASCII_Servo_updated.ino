...
byte ladd=0, radd=65;		// initial servo offsets:  unloaded bracket angles

const int saftyButtonPin = 13;
int saftyButtonState = 0;
...
void loop() {
  saftyButtonState = digitalRead(saftyButtonPin);

  if(saftyButtonState == LOW){
    ...
    byte actualLeft = left.read();
    byte actualRight = right.read();

    if(actualLeft != ladd && actualRight != radd){
      left.write(ladd);    // initial servo positions
      right.write(radd);
    }
  }
